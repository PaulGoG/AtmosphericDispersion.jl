...
imposes.
"""
function final_buoyant_rise(F::Real, u::Real, S::Real)
...
"""
function buoyant_rise(x::Real, F::Real, u::Real, S::Real)
    x ≥ 0 || throw(DomainError(x, "downwind distance cannot be negative"))
    final = final_buoyant_rise(F, u, S)
...
"""
function final_momentum_rise(Fₘ::Real, w₀::Real, D::Real, u::Real, S::Real)
...
"""
function momentum_rise(x::Real, Fₘ::Real, w₀::Real, D::Real, u::Real, S::Real)
    x ≥ 0 || throw(DomainError(x, "downwind distance cannot be negative"))
    final = final_momentum_rise(Fₘ, w₀, D, u, S)
...
Rise in metres from the semi-empirical law combining momentum and buoyancy,
    ...
    Δh = 3^(1/3) [ Fₘ x / ((1/3 + u/w₀)² u²) + F x² / (0.5 u³) ]^(1/3)

capped at the sum of the two final rises. Used where neither mechanism
dominates the other.
"""
function combined_rise(x::Real, F::Real, Fₘ::Real, w₀::Real, u::Real, S::Real, D::Real)
    x ≥ 0 || throw(DomainError(x, "downwind distance cannot be negative"))
    u > 0 || throw(DomainError(u, "wind speed at the release height must be positive"))
    w₀ > 0 || throw(DomainError(w₀, "exit velocity must be positive for the combined law"))
    final = final_momentum_rise(Fₘ, w₀, D, u, S) + final_buoyant_rise(F, u, S)
    transitional =
        3^(1 / 3) * (Fₘ * x / ((1 / 3 + u / w₀)^2 * u^2) + F * x^2 / (0.5 * u^3))^(1 / 3)
    return min(transitional, final)
end
